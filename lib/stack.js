...
module.exports = function (rc) {
  ...
  obj.apply = function (ids, cb, yarn) {
    fs.stat(packageJsonFile(), function (err, stat) {
      if (err) return cb(new Error('missing package.json'))
      ...
      var installthis = [
        { deps: dependencies, flags: [ '--save' ], type: 'prod' },
        { deps: devDependencies, flags: [ '--dev' ], type: 'dev' }
      ]
      ...
      async.eachSeries(installthis, function (item, next) {
        ...
        var args = yarn ? [ 'add' ] : ['install']
        args = yarn ? args.concat(modules).concat(item.flags) : args.concat(item.flags).concat(modules)
        yarn && item.type === 'prod' ? args.pop() : null
        var command = yarn ? 'yarn' : 'npm'
